...
class AbstractComponent extends React.Component {
    getModifier(...modifiers) {
        return modifiers.filter(m => m !== '').join(settings_1.default.getClasses().separator);
    }
    ...
    getBlockModifierWithComplexValueClassName(blockName, modifierKey, modifierValue, modifierMedia = '') {
        ...
        return Object.keys(modifierValues).filter((key) => modifierValues[key] !== '0').map((key) => {
            ...
            return this.getBlockModifierClassName(blockName, this.getModifier(newModifierKey, modifierValues[key], modifierMedia));
        }).join(' ');
    }
    getElementModifierWithComplexValueClassName(blockName, elementName, modifierKey, modifierValue, modifierMedia = '') {
        ...
        return Object.keys(modifierValues).filter((key) => modifierValues[key] !== '0').map((key) => {
            const newModifierKey = key === 'default' ? modifierKey : `${modifierKey}-${key}`;
            return this.getElementModifierClassName(blockName, elementName, this.getModifier(newModifierKey, modifierValues[key], modifierMedia));
        }).join(' ');
    }
    ...
    getElementName(blockName, elementName, modifiers = {}, isStatic = false) {
    ...
    getModifiers(blockName, elementName, modifiers, isStatic = false) {
        ...
        return Object.keys(modifiers).map((key) => {
            ...
            if (typeof value === 'object') {
                ...
                Object.keys(value).forEach((valueKey) => {
                    ...
                    if (elementName) {
                        if (className !== '') {
                            result.push(this.getElementModifierWithComplexValueClassName(blockName, elementName, key, className, media));
                        }
                        else {
                            result.push(this.getElementModifierClassName(blockName, elementName, this.getModifier(key, media)));
                        }
                    }
                    else {
                        if (className !== '') {
                            result.push(this.getBlockModifierWithComplexValueClassName(blockName, key, className, media));
                        }
                        else {
                            result.push(this.getBlockModifierClassName(blockName, this.getModifier(key, media)));
                        }
                    }
                });
                return result.join(' ');
